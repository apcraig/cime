...
! !EOP
   ...
   character(*),parameter ::   F01 = "('(shr_flux_atmOcn) ',a, d21.14)"
...
!        call ESMF_LogWrite(trim(subname)//" : n,tcx2 "//trim(tmpstr), ESMF_LOGMSG_INFO, rc=rc)
   ...
   if (debug > 0 .and. s_loglev > 0) then
      do n = 1,nMax
         if (mask(n) /= 0) then
            write(s_logunit, F01)'n,mask  = ',n,mask(n)
            write(s_logunit, F01)'n,zbot  = ',n,zbot(n)
            write(s_logunit, F01)'n,ubot  = ',n,ubot(n)
            write(s_logunit, F01)'n,vbot  = ',n,vbot(n)
            write(s_logunit, F01)'n,thbot = ',n,thbot(n)
            write(s_logunit, F01)'n,qbot  = ',n,qbot(n)
            write(s_logunit, F01)'n,tbot  = ',n,tbot(n)
            write(s_logunit, F01)'n,us    = ',n,us(n)
            write(s_logunit, F01)'n,vs    = ',n,vs(n)
            write(s_logunit, F01)'n,ts    = ',n,ts(n)
            write(s_logunit, F01)'n,lat   = ',n,lat(n)
            write(s_logunit, F01)'n,sen   = ',n,sen(n)
            write(s_logunit, F01)'n,taux  = ',n,taux(n)
            write(s_logunit, F01)'n,taux  = ',n,tauy(n)
            write(s_logunit, F01)'n,lwup  = ',n,lwup(n)
            write(s_logunit, F01)'n,evap  = ',n,evap(n)
         end if
      end do
   end if
