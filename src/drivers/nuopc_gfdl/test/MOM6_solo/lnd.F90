module LND
  ...
  subroutine SetServices(model, rc)
    ...
    call NUOPC_CompSetEntryPoint(model, ESMF_METHOD_RUN, &
      phaseLabelList=(/"update_land_model_fast"/), &
    ...
    call NUOPC_CompSpecialize(model, specLabel=model_label_Advance, &
      specPhaseLabel="update_land_model_fast", specRoutine=update_land_model_fast, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call NUOPC_CompSetEntryPoint(model, ESMF_METHOD_RUN, &
      phaseLabelList=(/"update_land_model_slow"/), &
      userRoutine=model_routine_Run, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call NUOPC_CompSpecialize(model, specLabel=model_label_Advance, &
      specPhaseLabel="update_land_model_slow", specRoutine=update_land_model_slow, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
  ...
  subroutine update_land_model_fast(model, rc) 
    type(ESMF_GridComp)  :: model 
    integer, intent(out) :: rc 
    ...
    type(ESMF_State)            :: importState, exportState 
    character(len=*),parameter  :: subname='(update_land_model_fast)' 
  ...
  subroutine update_land_model_slow(model, rc) 
    type(ESMF_GridComp)  :: model 
    integer, intent(out) :: rc 
    ...
    character(len=64)           :: timestr 
    type(ESMF_State)            :: importState, exportState 
    character(len=*),parameter  :: subname='(update_land_model_slow)' 
  ...
  subroutine SetRunClock(gcomp, rc)
    ...
    character(len=*),parameter :: subname='(lnd:SetRunClock)'
