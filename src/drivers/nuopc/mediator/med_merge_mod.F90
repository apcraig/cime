module med_merge_mod
  ...
  subroutine med_merge_fbx(FBout, n, FB, FBw, fldw, document, string, rc)
    ...
    ! Note - it is assumed that FB is initialized to zero the first time through
    ...
    ! Get name of the output field for index n in FBout
    call shr_nuopc_methods_FB_getNameN(FBout, n, FBoutfld, rc)
    if (shr_nuopc_methods_ChkErr(rc,__LINE__,u_FILE_u)) return

    ! Scan the output field name for an underbar
    n2 = scan(FBoutfld,'_')-1

    ! Determine the output field name prefix (before the underbar)
    FBoutpre  = trim(FBoutfld(1:n2))

    ! Determine the output field name suffix (after the underbar)
    FBoutname = trim(FBoutfld(scan(FBoutfld,'_'):))

    ! Obtain a pointer to the output field array, FBoutfld, in FBout
    call shr_nuopc_methods_FB_GetFldPtr(FBout, trim(FBoutfld), fldptr1=dp1, fldptr2=dp2, rank=lrank, rc=rc)
    if (shr_nuopc_methods_ChkErr(rc,__LINE__,u_FILE_u)) return

    ! Obtain the weight array, if a weight field bundle and a weight field is input
    if (present(FBw) .and. present(fldw)) then
      call shr_nuopc_methods_FB_GetFldPtr(FBw, trim(fldw), fldptr1=dpw1, fldptr2=dpw2, rc=rc)
      if (shr_nuopc_methods_ChkErr(rc,__LINE__,u_FILE_u)) return
    endif

    ! Loop over the input fields in the input field bundle
    ...
    do nf = 1,cntf
       ...
       ! Scan the input field name for an underbar
       n2 = scan(FBfld,'_')-1

       ! Determine the input field name prefix (before the underbar)
       FBpre  = trim(FBfld(1:n2))

       ! Determine the input field name suffix (after the underbar)
       ...
       if (trim(FBfld) == trim(FBoutfld)) then
          ...
          if (document) call ESMF_LogWrite(trim(subname)//":"//trim(lstring)//":"//trim(FBoutfld)//" ="//trim(FBfld), &
       ...
       elseif (index(FBoutpre,'x') > 0 .and. trim(FBname) == trim(FBoutname)) then
          ...
          ! If the output field bundle prefix (PBoutpre) contains a
          ! 'x' AND the input and output field bundle suffixes match
          ! then this is a merge.
          ...
          if (match) then
             call shr_nuopc_methods_FB_GetFldPtr(FB, trim(FBfld), fldptr1=dpf1, fldptr2=dpf2, rc=rc)
             if (shr_nuopc_methods_ChkErr(rc,__LINE__,u_FILE_u)) return
