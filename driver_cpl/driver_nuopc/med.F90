...
! Rof will pick up the grid from the first field exported to Rof
  ...
  subroutine MedPhase_prep_med2atm(gcomp, rc)
    ...
    !-------------------------
    ! copy scalar data to State and synchronize data in export and import state
    !-------------------------

    call med_method_CopyInfodataToState(infodata,NState_AtmExp,'cpl2atm',is_local%wrap%mpicom,rc)
    if (med_method_ChkErr(rc,__LINE__,__FILE__)) return
    call med_method_CopyInfodataToState(infodata,NState_AtmImp,'cpl2atm',is_local%wrap%mpicom,rc)
    if (med_method_ChkErr(rc,__LINE__,__FILE__)) return
  ...
  subroutine MedPhase_prep_med2ocn(gcomp, rc)
    ...
    call med_method_CopyInfodataToState(infodata,NState_OcnExp,'cpl2ocn',is_local%wrap%mpicom,rc)
    if (med_method_ChkErr(rc,__LINE__,__FILE__)) return
    call med_method_CopyInfodataToState(infodata,NState_OcnImp,'cpl2ocn',is_local%wrap%mpicom,rc)
    if (med_method_ChkErr(rc,__LINE__,__FILE__)) return
  ...
  subroutine MedPhase_post_atm2med(gcomp, rc)
    ...
    call med_method_CopyStateToInfodata(NState_AtmImp,infodata,'atm2cpl',is_local%wrap%mpicom,rc)
    if (med_method_ChkErr(rc,__LINE__,__FILE__)) return
  ...
  subroutine MedPhase_post_ocn2med(gcomp, rc)
    ...
    call med_method_CopyStateToInfodata(NState_OcnImp,infodata,'ocn2cpl',is_local%wrap%mpicom,rc)
    if (med_method_ChkErr(rc,__LINE__,__FILE__)) return
