module ESM
  ...
  subroutine SetModelServices(driver, rc)
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      ...
      return  ! bail out
#else
!    call ESMF_TimeIntervalSet(timeStep, d=1, rc=rc) ! 1 day timesteps
!    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
!      line=__LINE__, &
!      file=__FILE__)) &
!      return  ! bail out
!    internalClock = ESMF_ClockCreate(seq_timemgr_EClock_d, rc=rc)
!    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
!      line=__LINE__, &
!      file=__FILE__)) &
!      return  ! bail out
...
!      return  ! bail out
!    call ESMF_GridCompSet(driver, clock=internalClock, rc=rc)
!    call ESMF_GridCompSet(driver, clock=seq_timemgr_Eclock_d, rc=rc)
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      ...
      return  ! bail out
#endif
    call shr_nuopc_stuff_ClockTimePrint(seq_timemgr_Eclock_o,subname//'EClock_base',rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
  ...
  subroutine SetRunSequence(driver, rc)
    ...
    call NUOPC_DriverAddRunElement(driver, slot=1, &
      compLabel="MED", phaseLabel="MedPhase_prep_ocn", rc=rc)  ! MED-slow
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
#endif
    call NUOPC_DriverAddRunElement(driver, slot=1, &
      compLabel="MED", phaseLabel="MedPhase_prep_med2ocn", rc=rc)  ! MED-slow
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    call NUOPC_DriverAddRunElement(driver, slot=2, &
      compLabel="MED", phaseLabel="MedPhase_prep_atm", rc=rc) ! MED-fast-before
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
#endif
    call NUOPC_DriverAddRunElement(driver, slot=2, &
      compLabel="MED", phaseLabel="MedPhase_prep_med2atm", rc=rc)  ! MED-slow
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      ...
      return  ! bail out
    call NUOPC_DriverAddRunElement(driver, slot=2, &
      compLabel="MED", phaseLabel="MedPhase_post_atm2med", rc=rc)  ! MED-slow
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
#if (1 == 1)
    call NUOPC_DriverAddRunElement(driver, slot=2, &
      compLabel="MED", phaseLabel="MedPhase_accum_fast", rc=rc)  ! MED-fast-after
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      ...
      return  ! bail out
    call NUOPC_DriverAddRunElement(driver, slot=1, &
      compLabel="MED", phaseLabel="MedPhase_post_ocn2med", rc=rc)  ! MED-slow
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    call shr_nuopc_stuff_ClockTimePrint(seq_timemgr_Eclock_o,subname//'EClock_o',rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call shr_nuopc_stuff_ClockTimePrint(seq_timemgr_Eclock_a,subname//'EClock_a',rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
